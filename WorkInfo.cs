   ...
   public class WorkInfo
    {
        ...
        public Model.WorkInfo GetModel(int id)
        {
            ...
            {
                string sql = "select workinfo.*,activity.activityName from workinfo join activity on workinfo.activityId=activity.activityId where workIg=@id";
        ...
        public IEnumerable<Model.WorkInfoNo> GetPage(Model.WorkPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by uploadTime desc) as num,workinfo.* where activityId in @activityIds)";
        ...
        public int Update(Model.WorkInfo workinfo) 
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workName=@workName,workPicture=@workPiture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerify=@workVerify,userName=@userName,activityId=@activityId,recommend=@recommend,recommendTime=@recommendTime where workId=@workId";
